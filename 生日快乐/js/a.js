...
function Particles(param) {
	...
	this.animate = function() {
		// 做动效
		console.log("animate "+ image.isLoaded + "; this=" + this + "; that="+that)
		if(image.isLoaded) {
			Particles.prototype._animate(param.delay);
		} else {
			setTimeout(that.animate, 100);
		}
	}
};
	...
	_calculate: function(param) {
		...
		for(var i = 0; i < cols; i++) {
			...
			for(var j = 0; j < rows; j++) {
				...
				// 计算(i,j)在数组中的R的坐标值
				pos = [(j*single_h - 1)*image.w + (i*single_w - 1)]*4 + 1;
				...
				if(data[pos] <= 10)	{
					...
					this.array.push({
						// x0,y0起始坐标，x1,y1终点坐标
						x0: param.startX*(Math.random() + 0.5),
						y0: param.startY*(Math.random() + 0.5),
